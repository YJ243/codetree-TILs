...
void move(int x, int y){
    //cout << x << ' ' << y << "에 있는 참가자 옮기기 " << '\n';
    int eX = exit_loc.first, eY = exit_loc.second;
    int minL = abs(eX-x) + abs(eY-y), tX=-1, tY=-1;
    for(int d=0; d<4; d++){
...
bool CanMakeSquare(int x, int y, int len){
