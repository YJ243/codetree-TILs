...
void MovePeopleInGrid(){                    // 격자 안의 사람들을 이동하는 함수
    for(int i=1; i<=min(elapsed_time-1, m); i++){      // 1번부터 격자 안의 모든 사람 확인하기
        ...
        if(!isArrived){     // 만약 아직 편의점에 도착하지 못했다면, 이동하기
            ...
            pair<int, int> target = FindTargetLoc(i);           // 현재 이동하려는 사람이 가야할 편의점 좌표 
...
void Simulation(){
    ...
    // 1. 현재 격자 내 사람들 이동시키기
    MovePeopleInGrid();
    // 2. 모두 이동한 뒤 편의점에 도착한 사람 칸을 절대 지나갈 수 없다고 표시하기
    CheckArriveToStore();
    // 3. t<=m이면 격자 밖에 있는 사람, 즉 현재 초를 번호로 가지고 있는 사람을 이동시키기
    if(elapsed_time <= m){
        MoveOnePerson();
    }
}
