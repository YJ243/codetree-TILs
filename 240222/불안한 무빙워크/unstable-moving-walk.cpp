...
void MovePersonInWalk(){
    ...
    if(!last_person){    // 맨 마지막 자리에 사람이 없다면
        ...
        for(int i=(n*2)-1; i >= 1; i--){
            ...
            //cout << "옮기고 난 후 people[i]: " << i << "번째는" <<people[i] << "이후 people[i-1]" << people[i-1] << "\n";
            safety[i]--;
        }
    }
...
void Simulate(){
    // Step 1. 무빙워크 한 칸 회전
    //cout << "before moving walk, people state: " ;
    //for(int i=0; i<2*n; i++){
    //    cout << people[i] << ' ';
    //}
    //cout << '\n';
    MoveMovingWalk();
    //cout << "before moving people, people state: "; 
    //for(int i=0; i<2*n; i++){
    //    cout << people[i] << ' ';
    //}
    //cout << '\n';
    ...
    MovePersonInWalk();
    //cout << "after moving people, people state: " << '\n';
    //for(int i=0; i<2*n; i++){
    //    cout << people[i] << ' ';
    //}
    //cout << '\n';
