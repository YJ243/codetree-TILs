...
// maze[i][j]
// 0 -> 빈칸
// -1 ~ -9 -> 벽을 의미하고, 내구도는 양수 버전 (예, -5 => 내구도가 5인 벽)
// -10 -> 출구
...
pair<int, int> findExit(){  // 출구의 좌표를 돌려주는 함수
    for(int i=1; i<=N; i++){
        for(int j=1; j<=N; j++){
            if(maze[i][j] == -10){ // 만약 특정 격자에 -10이 적혀있다면, 출구를 의미한다.
                return {i,j};
            }
        }
    }
...
void rotate(){      // 출구와 참가자를 포함한 회전
    // 1. 정사각형의 크기를 먼저 결정하자
    // 가장 작은 정사각형의 크기 = 출구 & (출구와 가장 가까운 참가자)
    int minDist = 1000000;
    auto ex = findExit();
    for(int i=1; i<=N; i++){
        for(int j=1; j<=N; j++){
