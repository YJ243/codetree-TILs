...
void Bump(int idx, int x, int y, int d, int len){    // (x,y)에 있는 산타를 d 방향으로 len만큼 밀어내는 함수
    ...
    if(!ExistSanta(nx, ny)){
        // 산타 위치 업데이트 하기
        //cout << idx<<"번 위치를 이제 " << nx << ' ' << ny << "로 이동할 수 있음\n";
        santa[idx] = make_pair(nx, ny);
        return;
    }
    // 새로운 곳에 또 다른 산타가 있다면
    else{
        //cout << idx<<"번 움직이는데 새로운 곳에 산타가 있음\n";
...
void MoveSanta(int idx, int turn){    // idx번 산타를 움직이는 함수
    ...
    // 3.  충돌 처리하기
    if(rx == tX && ry == tY){
        //cout << idx<<"번은 산타와 충돌함" << tX << ' ' << tY << "로 왔고, " << tD<<"방향으로 옴\n";
        //cout << (tD+2)%4 << "방향으로 " << D << "만큼 가야 함"<<'\n';
        Bump(idx, tX, tY, (tD+2)%4, D);
        faint[idx] = turn+1;
        score[idx] += D;
        //cout << "이제 " << idx << "번의 위치는  : " << santa[idx].first << ' ' << santa[idx].second << '\n';
    }
}
...
void Simulate(int turn){    // turn 번째 시뮬레이션 진행하는 함수
    ...
    for(int i=1; i<=p; i++){
        ...
        //cout << turn << "번째 " << i << "번 산타의 위치는 : " << santa[i].first << ' ' <<santa[i].second << '\n';
    }
