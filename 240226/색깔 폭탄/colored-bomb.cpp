...
pair<int, int> bfs(){ 
    ...
    while(!q.empty()){
        ...
        for(int d=0; d<4; d++){
            ...
            if(CanGoFirst(nx, ny, grid[curr.first][curr.second])){
                visited[nx][ny] = true;
                curr_cnt++;
...
void delete_bomb(int color_num){
    while(!q.empty()){
        ...
        //cout << curr.first << ' ' << curr.second << ' ' << color_num << '\n';
            ...
            if(CanGoSecond(nx, ny, color_num)){
                ...
                if(grid[nx][ny] == 0){
                    for(int k=0; k<(int)red_loc.size(); k++){
                        if(red_loc[k].first == nx && red_loc[k].second == ny){
    ...
    for(int i=0; i<n; i++){
        for(int j=0; j<n; j++){
            if(grid[i][j] > 0 && !visited[i][j]){     // 만약 색깔 폭탄이라면
                ...
                //cout << i << ' ' << j << "에서 시작한 " << grid[i][j] << "와 같은 색을 가진 묶음에서 ";
                //cout << "개수는 "<<curr_cnt<<"이고, std_point는 " << std_point.first << ' ' << std_point.second << "임\n";
            }
        }
    }
    /*
        ...
        for(int d=0; d<4; d++){
            ...
            if(InRange(sr, sc) && !visited[sr][sc] && grid[sr][sc] > 0){
                ...
                for(int j=0; j<(int)red_loc.size(); j++){
                    grid[red_loc[j].first][red_loc[j].second] = false;
                }
            }
        }
    }
